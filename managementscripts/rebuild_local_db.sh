...
LATEST_FILE=`aws s3 ls s3://dc-ee-short-term-backups/every_election/ | sort | tail -n 1 | rev | cut -d' ' -f 1 | rev`
aws s3 cp s3://dc-ee-short-term-backups/every_election/$LATEST_FILE - | pg_restore -d every_election --if-exists --clean
