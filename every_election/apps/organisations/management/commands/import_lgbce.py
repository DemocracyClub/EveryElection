...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            "org", action="store", help="3-letter organisation code e.g: KHL"
        )
        parser.add_argument(
            "-s",
            "--s3",
    ...
    def get_division_set(self, org_code):
        ...
        if not divset.divisions.all():
            error = (
                "Candidate DivisionSet '%s' has no Divisions. Can not import boundaries"
                % (divset)
            )
        ...
        if divset.has_related_geographies:
            error = (
                "Candidate DivisionSet '%s' already has related Geographies. Can not import boundaries"
                % (divset)
            )
            raise Exception(error)
        ...
        if divset.end_date:
            error = (
                "Candidate DivisionSet '%s' already has end_date %s. Expected NULL"
                % (divset, divset.end_date)
            )
            raise Exception(error)
